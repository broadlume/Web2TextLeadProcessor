...
export async function GetRetailerByID(universalId: UUID): Promise<NexusRetailer | null> {
    ...
    try {
        ...
        logger.child({ label: "NexusRetailerAPI:GetRetailerByID" }).warn("Error fetching retailer from Nexus");
        logger.child({ label: "NexusRetailerAPI:GetRetailerByID" }).error(e);
        throw e;
    }
}
