...
export async function ListDeployments(): Promise<RestateDeployment[]> {
    ...
    const json = await ky.get(restateURL.toString(), { retry: 0 }).json<{ deployments: RestateDeployment[] }>();
...
export async function DeleteDeployment(
    ...
    if (options.force !== true) {
        ...
        const serviceUsingDeployment = services.find((s) => s.deployment_id === deploymentId);
