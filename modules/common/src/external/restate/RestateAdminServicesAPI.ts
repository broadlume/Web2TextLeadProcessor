...
export async function ListServices(): Promise<RestateService[]> {
    ...
    const json = await ky.get(restateURL.toString(), { retry: 0 }).json<{ services: RestateService[] }>();
 ...
 */
export async function ModifyServiceState(service: string, objectKey: string, state: any): Promise<void> {
    const restateURL = new URL(process.env.RESTATE_ADMIN_URL as string);
