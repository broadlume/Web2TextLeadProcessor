...
export async function FindConversationsFor(
    ...
    conversations = conversations.filter((c) => statuses.includes(c.conversationState));
    const conversationInstances = await Promise.all(
        conversations.map(async (c) => await twilioClient.conversations.v1.conversations(c.conversationSid).fetch()),
    );
