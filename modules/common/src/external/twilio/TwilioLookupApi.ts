...
async function _lookupPhoneNumber(twilioClient: Twilio, phoneNumber: E164Number): Promise<PhoneNumberInstance> {
    const lookup = await twilioClient.lookups.v2.phoneNumbers(phoneNumber).fetch({ fields: "line_type_intelligence" });
    return lookup;
}
