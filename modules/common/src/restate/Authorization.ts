 ...
 */
async function CheckAPIKeyStatus(endpoint: string, auth: string | undefined): Promise<ValidationStatus> {
    if (auth == null) {
        return {
            Name: "API Key",
            Status: "NONEXISTANT",
    ...
    if (z.string().uuid().safeParse(key).success === false) {
        return {
            Name: "API Key",
            Status: "INVALID",
            Reason: "API Key is not a valid UUIDv4",
        };
    }
    if (process.env.INTERNAL_API_TOKEN != null && key === process.env.INTERNAL_API_TOKEN) {
    ...
    if (apiKey.AuthorizedEndpoints.includes("*") || apiKey.AuthorizedEndpoints.includes(endpoint))
        return { Name: "API Key", Status: "VALID" };
    return {
        Name: "API Key",
        Status: "INVALID",
 ...
 * @param auth the authorization header value
 */
export async function CheckAuthorization(ctx: restate.Context, endpoint: string, auth: string | undefined) {
