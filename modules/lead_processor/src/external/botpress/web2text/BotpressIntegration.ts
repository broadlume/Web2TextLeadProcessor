...
> {
    ...
    async create(
        ...
        const botpressAIData = await context.run("Fetching Botpress AI Summary", async () => {
            const response = await BotpressTableAPI.FindTableRows("Int_Connor_Conversations_Table", process.env.BOTPRESS_BOT_ID!, process.env.BOTPRESS_WORKSPACE_ID!, {
                "filter": {
    ...
    async sync(state: BotpressIntegrationState, context: ObjectSharedContext<SubmittedLeadState<Web2TextLead>>): Promise<BotpressIntegrationState> {
        return state;
    }
    async close(state: BotpressIntegrationState, context: ObjectSharedContext<SubmittedLeadState<Web2TextLead>>): Promise<BotpressIntegrationState> {
        return state;
    }
