...
import { Web2TextLeadIntoDHQStoreInquiry, Web2TextMessageIntoDhqComment } from "#external/dhq/web2text/APIConverters";
...
export class DHQIntegration extends IExternalIntegration<SubmittedLeadState<Web2TextLead>, DHQIntegrationState> {
    Name = "DHQ";
    ...
    async create(
        ...
        const store = await context.run(
            ...
            async () => await NexusStoresAPI.GetRetailerStoreByID(leadState.Lead.LocationId),
        );
        if (store === null) {
            return {
                ...state,
