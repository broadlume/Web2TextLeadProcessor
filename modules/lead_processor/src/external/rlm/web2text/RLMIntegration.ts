...
import { Web2TextLeadIntoRLMLead, Web2TextMessageIntoRLMNote } from "#external/rlm/web2text/APIConverters";
...
export class RLMIntegration extends IExternalIntegration<SubmittedLeadState<Web2TextLead>, RLMIntegrationState> {
    Name = "RLM";
    ...
    private async getRLMLocationName(
        ...
        const storeInformation = await NexusStoresAPI.GetRetailerStoreByID(nexusLocationId);
        if (storeInformation?.universal_id == null) return null;
        return rlmLocationsMapping.find((mapping) => mapping.dhq_store_id === storeInformation?.universal_id) ?? null;
    }
    async create(
        ...
        const response = await context.run(
            ...
            async () =>
                await RLMLeadsAPI.CreateLead(
                    leadState.LeadId,
                    new Web2TextLeadIntoRLMLead(leadState, rlmLocationMapping?.location_name ?? undefined),
                    rlm_api_key!,
                ).catch(
                    (e) =>
                        ({
        ...
        const newState: RLMIntegrationState = {
            ...
            Data: {
                ...
                LocationName: rlmLocationMapping?.location_name ?? "Per Pipeline configuration",
