...
import { GetRunningEnvironment, isDeployed, isProductionAndDeployed } from "common";
...
import { TwilioConversationHelpers, TwilioProxyAPI } from "common/external/twilio";
...
import { IsPhoneNumberOptedOut } from "#lead/web2text/validation";
import { LeadVirtualObject } from "#restate";
import { DealerCloseMessage, DealerGreetMessage, SystemGreetingMessage } from "./Web2TextMessagingStrings";
...
export class TwilioIntegration extends IExternalIntegration<SubmittedLeadState<Web2TextLead>, TwilioIntegrationState> {
    readonly CONVERSATION_CLOSED_TIMER = isProductionAndDeployed() ? "P30D" : "P1D";
    ...
    async create(
        ...
        const locationInformation = await context.run("Get retailer store from Nexus", async () =>
            NexusStoresAPI.GetRetailerStoreByID(leadState.Lead.LocationId),
        );
        if (locationInformation === null) {
            throw new Error(`Location info is missing in Nexus for location ID '${leadState.Lead.LocationId}'`);
        }
        const dealerInformation = await context.run(
            "Get retailer info from Nexus",
            async () => await NexusRetailerAPI.GetRetailerByID(leadState.UniversalRetailerId),
        );
        ...
        const { isNewConversation, conversation } = await context.run(
            ...
            async () =>
                this.createWeb2TextConversation(leadState, storePhoneNumber, dealerInformation, locationInformation),
            {
                maxRetryAttempts: 3,
            },
        );
        if (!isNewConversation) {
            context.console.info(`Found pre-existing Twilio Conversation: ${conversation.sid}`, {
                _meta: 1,
            ...
            context.console.info(`Created new Twilio Conversation: ${conversation.sid}`, {
                _meta: 1,
                ConversationSID: conversation.sid,
        ...
        await context.run("Send dealer messaging", async () => {
            await this.sendSystemMessage(conversation.sid, dealerMessaging, leadState.Lead.PhoneNumber, false);
        });
        await context.run("Send customer message", async () => {
            ...
            await this.sendSystemMessage(conversation.sid, systemMessaging, storePhoneNumber, false);
        });
    ...
    async sync(
        ...
        const conversation = await context.run("Fetching Twilio conversation", async () =>
        ...
        if (conversation == null || conversation.state === "closed") {
            context.objectSendClient(LeadVirtualObject, lead.LeadId).close(
                { reason: "Inactivity" },
        ...
        const phoneNumbers = participants.map((p) => (p.messagingBinding as any)?.address).filter((p) => p != null);
        if (phoneNumbers.length <= 1) {
            // Close the lead if no phone numbers left in conversation
            context.objectSendClient(LeadVirtualObject, lead.LeadId).close(
                { reason: "One or fewer participants left in conversation" },
                restate.rpc.sendOpts({
                    headers: {
                        authorization: process.env.INTERNAL_API_TOKEN!,
                    },
    ...
    async close(
        ...
        await context.run("Update twilio conversation", async () => {
            await this.twilioClient.conversations.v1.conversations(conversationID).update(update);
        });
        return {
            ...state,
            SyncStatus: "CLOSED",
    ...
    private async sendSystemMessage(
        ...
        const twilioMessage = await this.twilioClient.conversations.v1.conversations(conversationSid).messages.create({
            author: author,
            body: message,
    ...
    private async createWeb2TextConversation(
        ...
        const preExistingConversation = await TwilioConversationHelpers.FindConversationsFor(
            this.twilioClient,
        ...
        if (preExistingConversation) {
            ...
            conversation = await TwilioProxyAPI.CreateSession([leadState.Lead.PhoneNumber, storePhoneNumber], {
        ...
        if (isDeployed()) {
            ...
            const activeWebhooks = await this.twilioClient.conversations.v1
                .conversations(conversation.sid)
                .webhooks.list();
            const hasSyncWebhook = activeWebhooks.find((w) => encodeURI(syncEndpoint.toString()));
            const hasCloseWebhook = activeWebhooks.find((w) => encodeURI(closeEndpoint.toString()));
            if (!hasSyncWebhook) {
                await this.twilioClient.conversations.v1.conversations(conversation.sid).webhooks.create({
                    "configuration.url": encodeURI(syncEndpoint.toString()),
                    "configuration.method": "POST",
                    target: "webhook",
                    "configuration.filters": ["onMessageAdded", "onMessageRemoved", "onMessageUpdated"],
                });
            }
            if (!hasCloseWebhook) {
                await this.twilioClient.conversations.v1.conversations(conversation.sid).webhooks.create({
