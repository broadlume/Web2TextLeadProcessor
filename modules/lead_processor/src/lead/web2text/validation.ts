...
export class Web2TextLeadValidator implements Validator<NonValidatedLeadState<Web2TextLead>> {
    ...
    public async validate(leadState: NonValidatedLeadState<Web2TextLead>): Promise<ValidationStatus> {
        ...
        const storePhoneNumber = parsePhoneNumber(
            (
                await this.ctx.run(
                    ...
                    async () => await NexusStoresAPI.GetRetailerStoreByID(lead.LocationId),
                )
        ...
        const customerPhoneStatus = await this.ctx.run(
            ...
            async () => await CheckPhoneNumberStatus(globalThis.TWILIO_CLIENT, lead.PhoneNumber),
        );
 ...
 */
export async function CheckIpAddressStatus(ipAddress: string): Promise<ValidationStatus> {
    return {
        Name: "IP Address",
        Status: "VALID",
    };
 ...
 */
export async function CheckClientStatus(universalId: UUID): Promise<ValidationStatus> {
...
export async function CheckLocationStatus(
    ...
    const phoneNumberStatus = await CheckPhoneNumberStatus(globalThis.TWILIO_CLIENT, locationPhone!.number);
...
export async function CheckPhoneNumberStatus(
    ...
    if (lineTypeError != null) {
        logger
            ...
            .warn(`Twilio line type intelligence responded with error code: ${lineTypeError}`, {
