...
export async function SyncWithDB(
    ...
    ctx.console.debug(`Begin DynamoDB lead state sync with direction: ${direction}`, { _meta: 1 });
    let synced = false;
    switch (direction) {
        case "SEND": {
        ...
        case "RECEIVE": {
            ...
            const { data, success, error } = await LeadStateSchema(leadTypeInfo.schema).safeParseAsync(lead);
            if (!success) {
                throw new restate.TerminalError(`Could not parse lead ID '${leadID}' from database`, {
    ...
    ctx.console.debug(`End DynamoDB lead state sync with direction: ${direction}`, { _meta: 1, Success: synced });
    return synced;
}
