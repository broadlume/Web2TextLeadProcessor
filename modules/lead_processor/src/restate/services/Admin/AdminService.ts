    ...
    handlers: {
        ...
        bulk: restate.handlers.handler(
            ...
            async (ctx: restate.Context, request: Record<string, string>): Promise<BulkEndpointResponse> => {
                ...
                const leads: SubmittedLeadState<any>[] = (await ctx.run("Scan LeadStates in DynamoDB", async () => {
                    let scan: Scan<any>;
                    try {
                        scan = LeadStateModel.scan(filter);
                    } catch (err) {
                        assert(is<Error>(err));
                        throw new restate.TerminalError(`Error parsing Filter parameter - ${err.message}`, {
                            errorCode: 400,
                            cause: err,
                        });
                    }
                    ...
                    return await scan
                        ...
                        .exec()
                        .catch((err) => {
                            throw new restate.TerminalError("Error scanning LeadState table", {
                                errorCode: 500,
                                cause: err,
                            });
                        });
                })) as SubmittedLeadState<any>[];
                let result: any[] = [];
                switch (parsed.data.Operation) {
                    case "CLOSE": {
                        if (parsed.data.Async === true) {
                            ...
                            const results: any[] = await restate.CombineablePromise.allSettled(
                                leads.map((lead) =>
                    ...
                    default:
                        throw new restate.TerminalError(`Invalid operation: '${parsed.data.Operation}'`);
                }
                ctx.console.log(
