...
function ValidateTwilioRequest(twilioHeader: string | undefined, data: object, endpoint: string) {
    if (twilioHeader == null) {
        throw new restate.TerminalError("Twilio auth header missing");
    }
    const thisUrl = new URL(`${TwilioWebhooks.name}/${endpoint}`, process.env.PUBLIC_RESTATE_INGRESS_URL);
    thisUrl.port = "";
    ...
    handlers: {
        sync: restate.handlers.handler(
            ...
            async (ctx: restate.Context, data: object) => {
                const twilioHeader =
                    ctx.request().headers.get("x-twilio-signature") ?? ctx.request().headers.get("X-Twilio-Signature");
                ...
                const conversation = await ctx.run(
                    ...
                    async () => await TWILIO_CLIENT.conversations.v1.conversations(data.ConversationSid).fetch(),
                );
        ...
        checkOptInStatus: restate.handlers.handler(
            ...
            async (
                ...
                if (phoneNumber === undefined) {
                    throw new restate.TerminalError("'PhoneNumber' in POST body is missing or invalid", {
                        errorCode: 400,
                    });
                }
                ...
                const optInNumbers = Object.keys(optOut.OptedOutNumbers) as E164Number[];
                return {
                    Status: "OPTED-OUT",
...
async function HandleOptInMessage(ctx: restate.Context, data: TwilioMessagingServiceBody) {
...
async function HandleOptOutMessage(ctx: restate.Context, data: TwilioMessagingServiceBody) {
    const conversations = await ctx.run("Find twilio conversation", async () =>
        TwilioConversationHelpers.FindConversationsFor(globalThis.TWILIO_CLIENT, data.From, ["active", "inactive"]),
    );
...
async function HandleClosedMessagingThread(
    ...
    const conversations = await ctx.run("Find twilio conversation", async () =>
        TwilioConversationHelpers.FindConversationsFor(globalThis.TWILIO_CLIENT, data.From, [
            "active",
            "closed",
            "inactive",
        ]),
    ...
    if (conversations.find((c) => c.state === "active" || c.state === "inactive")) return;
    ...
    if (data.From === storePhoneNumber) {
        ...
        closingMessage = CustomerCloseMessage(dealerName, dealerWebsite, storePhoneNumber);
    }
