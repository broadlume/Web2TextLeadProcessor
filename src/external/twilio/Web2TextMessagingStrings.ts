...
export function CustomerCloseMessage(dealerName?: string, dealerWebsiteURL?: string, dealerPhoneNumber?: string): string {
    ...
    if (dealerName === null || (dealerWebsiteURL == null && dealerPhoneNumber == null)) {
        return message;
    }
...
export function DealerCloseMessage(customerName: string, reason?: string) {
    let message = `This message thread with ${customerName} has expired, please reach out to the customer directly.`;
    if (reason) {
        message += `\n\nReason: ${reason}`;
    }
    return message;
